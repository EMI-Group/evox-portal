...
Empiece con la instrucción más sencilla:
...
> **Use EvoX para escribir un algoritmo que resuelva el siguiente problema: un robot de almacén se mueve desde la entrada hasta la salida y debe evitar los estantes a lo largo del camino. Construya un diseño de almacén fijo y reproducible; encuentre una ruta lo más corta posible, sin colisiones y suave; ejecute el experimento; y visualice el proceso de optimización.**
...
- `Workflow` conecta el algoritmo, el problema y el proceso de monitoreo.
...
> **Use EvoX para realizar un experimento de comparación de algoritmos multi-objetivo con un presupuesto unificado y 30 ejecuciones independientes, comparando NSGA-II, MOEA/D, HypE, AGE-MOEA y búsqueda aleatoria. Siga las prácticas experimentales habituales de IEEE TEVC para HV/IGD+, tamaños de efecto y análisis estadísticos no paramétricos, y genere un informe de visualización con curvas de convergencia, frentes de Pareto y limitaciones claramente declaradas.**

![Solicitud en lenguaje natural y resumen de Codex para la comparación multi-objetivo](./codex-for-evox-4.png)

Codex organiza el protocolo, mientras que cuatro algoritmos evolutivos basados en EvoX —NSGA-II, MOEA/D, HypE y el port local de AGE-MOEA— se evalúan bajo la misma interfaz de problema, tamaño de población, presupuesto de evaluación y protocolo de monitoreo. Se incluye una línea base independiente de búsqueda aleatoria como contexto. Las métricas, historiales de convergencia, conjuntos de soluciones finales y análisis estadísticos se registran dentro de la misma estructura de experimento.
...
### 5. Qué Evolucionar Depende de Usted
...
Considere una tarea de control clásica resuelta mediante neuroevolución:

> **Use EvoX para evolucionar un pequeño controlador basado en red neuronal sin retropropagación, permitiéndole equilibrar un péndulo invertido. Complete una implementación mínima ejecutable y genere la curva de entrenamiento y la animación de control final.**
...
> **Use EvoX y MAP-Elites para generar automáticamente un conjunto diverso de laberintos resolubles, mapee el paisaje de diversidad por la sinuosidad de la ruta y la riqueza de ramificaciones, y compare los resultados con la generación aleatoria bajo el mismo presupuesto.**
...
> **El experimento actual del péndulo invertido se ejecuta más lentamente en GPU que en CPU. Analice el cuello de botella de rendimiento; tensorice la evaluación de la población, la inferencia del controlador y la simulación del entorno en la GPU; compare el rendimiento de CPU y GPU antes y después de la optimización; y visualice los resultados.**
...
Empiece con una sola frase y explore su propio espacio evolutivo.
