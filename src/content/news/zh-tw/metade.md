...
MetaDE 採用雙層結構，包含**一個演化者**（上層）和**多個執行者**（下層）。演化者是一個 DE（或可能是其他演化演算法），負責最佳化 PDE 的參數。演化者族群中的每個個體 ![spacer.gif](/images/articles/metade-6.gif) x_i 對應一個唯一的參數配置 θ_i。這些配置被傳遞給 PDE 以實例化不同的 DE 變體，每個變體由一個執行者管理，並在給定的最佳化任務上獨立運行。每個執行者將其最佳適應度值 y^* 返回給演化者，演化者將該適應度值 y_i 分配給相應的個體 x_i。
...
![圖片2.png](/images/articles/metade-11.png)
