...
- `i18n/` — Centralized UI-string translation (`utils.ts` + `locales/*.json`, 13 files with exactly 176 keys each) and localized-content helpers (`getLocalizedArticles`/`getLocalizedNews`/`findLocalizedArticle`/`findLocalizedNews` with English fallback). See `i18n/CONTEXT.md`.
