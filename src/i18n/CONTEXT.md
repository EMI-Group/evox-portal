...
- Key naming: flat, dot-separated groups — `site.` (1), `meta.` (1), `nav.` (22), `hero.` (6), `news.` (12), `features.` (10), `examples.` (7), `footer.` (10), `lang.` (13), `community.` (9), `docs.` (9), `ecosystem.` (4), `genesis.` (35), `libs.` (19), `releases.` (10), `runtime.` (4), `tutorials.` (4).
- `libs.desc.*` keys (one per library: evox, evogit, evogp, evorl, evomo, tensorneat, metade, evoxbench, istratde, evogo, evonas, evocmo, autopso, evococo) are consumed by `src/pages/libs.astro` and `src/pages/[locale]/libs/index.astro` to translate `src/content/libs.json` descriptions — every lib in the JSON needs a matching `libs.desc.<id>` key in every locale.
