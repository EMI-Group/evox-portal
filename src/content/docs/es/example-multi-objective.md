...
A menudo preferimos ejecutar nuestro codigo en una GPU para una ejecucion mas rapida. Sin embargo, si una GPU no esta disponible, ejecutar en CPU tambien es aceptable.
...
Con esta configuracion en su lugar, ahora podemos comenzar a optimizar. Configuramos el algoritmo multiobjetivo para optimizar durante 100 pasos en este problema.

```python
# Ejecutar el flujo de trabajo durante 100 pasos
t = time.time()
workflow.init_step()
for i in range(100):
    compiled_step()
