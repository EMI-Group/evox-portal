---
title: "사용자 정의 알고리즘으로 HPO 배포하기"
order: 6
section: "developer"
---

# 사용자 정의 알고리즘으로 HPO 배포하기

이 장에서는 전체 워크플로우보다는 세부 사항에 중점을 두어 사용자 정의 알고리즘으로 HPO를 배포하는 데 초점을 맞출 것입니다. HPO 배포에 대한 간단한 소개는 [tutorial](/ko/tutorials/practical-examples)에 제공되며, 미리 읽어보는 것을 강력히 권장합니다.

## 알고리즘을 병렬화 가능하게 만들기

내부 알고리즘을 문제(problem)로 변환해야 하므로, 내부 알고리즘이 병렬화 가능한 것이 중요합니다. 따라서 알고리즘에 대한 일부 수정이 필요할 수 있습니다.

1. 알고리즘은 자체 속성에 대해 in-place 연산을 수행하는 메서드를 가져서는 안 됩니다.
...
2. 코드 로직이 Python 제어 흐름(control flow)에 의존하지 않아야 합니다.
...
HPO 작업에서는 `HPOMonitor`를 사용하여 각 내부 알고리즘의 지표를 추적해야 합니다. `HPOMonitor`는 표준 `monitor`와 비교하여 `tell_fitness`라는 하나의 메서드만 추가합니다. 이 추가 기능은 HPO 작업이 종종 다차원적이고 복잡한 지표를 포함하기 때문에 지표 평가에 더 큰 유연성을 제공하기 위해 설계되었습니다.

사용자는 `HPOMonitor`의 하위 클래스를 생성하고 `tell_fitness` 메서드를 오버라이드하여 사용자 정의 평가 지표를 정의하기만 하면 됩니다.

또한 다목적 문제에 대한 'IGD' 및 'HV' 지표 계산과 단일 목적 문제에 대한 최소값을 지원하는 간단한 `HPOFitnessMonitor`도 제공합니다.

## 간단한 예제

여기서는 EvoX로 HPO를 사용하는 간단한 예제를 보여드리겠습니다. `PSO` 알고리즘을 사용하여 sphere 문제를 해결하기 위한 기본 알고리즘의 최적 하이퍼파라미터를 탐색할 것입니다.
...
다음으로 간단한 sphere 문제를 정의합니다. 이는 일반적인 `problems`와 차이가 없다는 점에 유의하세요.
...
다음으로 알고리즘을 정의합니다. `torch.cond` 함수를 사용하고 병렬화 가능한지 확인합니다. 구체적으로 in-place 연산을 수정하고 Python 제어 흐름을 조정합니다.
...
Python 제어 흐름을 처리하기 위해 [`torch.cond`](https://pytorch.org/docs/stable/generated/torch.cond.html)를 사용합니다. 그 다음 `StdWorkflow`를 사용하여 `problem`, `algorithm`, `monitor`를 래핑할 수 있습니다. 그런 다음 `HPOProblemWrapper`를 사용하여 `StdWorkflow`를 HPO 문제로 변환합니다.
...
`HPOProblemWrapper`가 우리가 정의한 하이퍼파라미터를 올바르게 인식하는지 테스트할 수 있습니다. 7개의 인스턴스에 대해 하이퍼파라미터를 수정하지 않았으므로 모든 인스턴스에서 동일해야 합니다.
...
우리만의 하이퍼파라미터 값 세트를 지정할 수도 있습니다. 하이퍼파라미터 세트의 수는 `HPOProblemWrapper`의 인스턴스 수와 일치해야 한다는 점에 유의하세요. 사용자 정의 하이퍼파라미터는 값이 `Parameter`로 래핑된 딕셔너리로 제공되어야 합니다.
...
이제 `PSO` 알고리즘을 사용하여 `ExampleAlgorithm`의 하이퍼파라미터를 최적화합니다. `PSO`의 개체군 크기(population size)는 인스턴스 수와 일치해야 합니다. 그렇지 않으면 예상치 못한 오류가 발생할 수 있습니다. 이 경우 `HPOProblemWrapper`는 딕셔너리를 입력으로 요구하므로 외부 워크플로우에서 솔루션을 변환해야 합니다.
