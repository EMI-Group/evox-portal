---
title: "Custom algorithms and problems in EvoX"
order: 5
section: "developer"
---
...
The `Algorithm` class is inherited from `ModuleBase`.
...
> **Note:**
> The static initialization can still be written in the `__init__` while the mutable submodule(s) initialization cannot. Therefore, multiple calls of `setup` for repeated initializations are possible if the overwritten `setup` method invokes the `setup()` of `ModuleBase` first.
> 
> If such `setup` method in `ModuleBase` is not suitable for your algorithm, you can override the `setup` method when you create your own algorithm class.


## Problem class

The `Problem` class is also inherited from `ModuleBase`.
