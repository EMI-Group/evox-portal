...
Чтобы запустить программу на определённом GPU, вы можете использовать переменную окружения `CUDA_VISIBLE_DEVICES`. Например, чтобы запустить программу на втором GPU, используйте:
