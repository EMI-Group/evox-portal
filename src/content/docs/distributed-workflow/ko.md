...
```
> **중요:**
> 모든 torch 또는 numpy 연산 **이전에** 모든 난수 생성기의 시드를 설정해야 합니다. 이는 연산이 수행되기 전에 난수 생성기가 알려진 상태에 있도록 보장합니다.
2. `torch.distributed` 또는 `torchrun` 명령어를 사용하여 스크립트를 실행하세요. 예를 들어:
```bash
torchrun
    --standalone
    --nnodes=1
