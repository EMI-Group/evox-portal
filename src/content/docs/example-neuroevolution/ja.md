...
この例では、まず個体群ベースのニューロエボリューションアルゴリズムをテストし、代表として粒子群最適化（`PSO`）を使用します。ニューロエボリューションの設定は他の最適化タスクと同様です。初期化を完了するために、問題（problem）、アルゴリズム（algorithm）、モニター（monitor）、ワークフロー（workflow）と、それぞれの`setup()`関数を定義する必要があります。
