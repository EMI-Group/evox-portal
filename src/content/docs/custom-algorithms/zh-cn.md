...
在大多数传统的演化计算（EC）库中，算法通常在内部调用目标函数，其布局如下：
...
> **注意：**
> 静态初始化仍然可以写在 `__init__` 中，但可变子模块的初始化则不能。因此，如果重写的 `setup` 方法首先调用了 `ModuleBase` 的 `setup()`，那么多次调用 `setup` 进行重复初始化是可能的。
> 
> 如果 `ModuleBase` 中的这种 `setup` 方法不适合您的算法，您可以在创建自己的算法类时重写 `setup` 方法。
...
这里我们给出一个**实现 PSO 算法来解决 Sphere 问题**的示例。
