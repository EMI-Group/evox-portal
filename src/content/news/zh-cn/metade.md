...
MetaDE 采用双层结构，包含**一个演化器**（上层）和**多个执行器**（下层）。演化器是一个 DE（或者是其他演化算法），负责优化 PDE 的参数。演化器种群中的每个个体 ![spacer.gif](/images/articles/metade-6.gif) x_i 对应一个唯一的参数配置 θ_i。这些配置被传递给 PDE 以实例化不同的 DE 变体，每个变体由一个执行器管理，并在给定的优化任务上独立运行。每个执行器将其最佳适应度值 y^* 返回给演化器，演化器将该适应度值 y_i 分配给相应的个体 x_i。
