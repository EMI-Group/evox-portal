...
В этой главе представлено несколько полных практических примеров, демонстрирующих применение знаний из предыдущих глав. Мы создадим проект оптимизации с нуля и покажем, как EvoX может быть интегрирован с другими инструментами. Эти примеры охватывают различные типы задач, что поможет вам применять EvoX в реальных сценариях.

---

## Пример 1: Однокритериальная оптимизация
...
**Шаг 2: Настройка рабочего процесса (Workflow)**
...
Алгоритм PSO находит решение, близкое к оптимальному (в районе начала координат), как и ожидалось.

---

## Пример 2: Многокритериальная оптимизация
...
Фронт Парето находится между $x = 0$ (оптимум для $f_1$) и $x = 2$ (оптимум для $f_2$).

**Шаг 1: Настройка окружения**

Убедитесь, что у вас установлен EvoX с поддержкой NSGA-II.

**Шаг 2: Определение пользовательской задачи**

В EvoX встроено множество тестовых многокритериальных задач, но для этого примера мы определим собственную задачу для оптимизации двух функций:
...
**Задача**: Настройка параметров `C` и `max_iter` классификатора логистической регрессии на наборе данных breast cancer для максимизации точности на валидационной выборке.
...
Эти практические примеры иллюстрируют, как EvoX может эффективно применяться в различных областях: от математических тестовых функций до рабочих процессов машинного обучения. Как только вы освоите базовую структуру — **Algorithm + Problem + Monitor + Workflow** — вы сможете адаптировать EvoX практически для любой задачи оптимизации.